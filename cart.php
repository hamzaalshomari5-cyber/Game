...
if ($action === 'checkout') {
    ...
    foreach ($s['items'] as $it) {
        ...
        db()->prepare("INSERT INTO orders (user_id,product_id,product_name,qty,player_id,price,total,uuid)
            VALUES (?,?,?,?,?,?,?,?)")
            ->execute([$U['id'], $it['product_id'], $p['name'], $it['qty'], $it['player_id'], $p['price'], $total, $uuid]);
